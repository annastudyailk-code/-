...
8. **De-slop** — run [humanizer-slop.md](../../shared/aaron-marketing-skills/references/humanizer-slop.md) over rewritten titles/descriptions to strip AI tells before handoff.

Never invent a GTIN, price, stock count, or product spec to fill a gap; if a required attribute is missing, mark it `[needs source]` per item and submit any claim that needs a figure as an authorized `operation: propose` request through `registry-events.py` to `memory/events/claims.ndjson` — [offer-claims-registry](../../shared/aaron-marketing-skills/protocol/offer-claims-registry/SKILL.md) resolves the flags; only it may accept the canonical mutation.

**Scope guard**: this skill hardens the **product data** behind Shopping/PMax — titles, attributes, identifiers, disapproval hygiene, and feed-driven groups. It does **not** write text-ad copy or RSA units (that is [ad-creative-builder](../ad-creative-builder/SKILL.md)), does **not** compute or roll up the RQS or fire the O1/O2 vetoes (that is [ad-account-auditor](../ad-account-auditor/SKILL.md)), and does **not** fix the post-click page (that is [landing-optimizer](../../shared/aaron-marketing-skills/influencer/report/landing-optimizer/SKILL.md)).
...
On user confirmation, save to `memory/ad/product-feed-optimizer/YYYY-MM-DD-<catalog-or-goal>-feed.md` — see [Skill Contract](../../shared/aaron-marketing-skills/references/skill-contract.md) §Save Results Template. Store the triage table, the title/attribute conventions, and the hygiene rules; do not store the full raw catalog.
...
- [roas-benchmark.md](../../shared/aaron-marketing-skills/references/roas-benchmark.md) — the ROAS framework; this skill hardens the product data behind the **O (Offer)** dimension it scores (O1 claim integrity, O2 policy)
- [ad-account-auditor](../ad-account-auditor/SKILL.md) — scores the feed against ROAS and runs the O1/O2 vetoes (next skill)
- [CONNECTORS.md](../../shared/aaron-marketing-skills/CONNECTORS.md) — keyless export recipes for `~~ad platform` (feed + diagnostics), `~~ecommerce`
- [Humanizer Slop Check](../../shared/aaron-marketing-skills/references/humanizer-slop.md) — pre-handoff pass that strips AI-slop phrasing from rewritten titles/descriptions
- [SECURITY.md](../../shared/aaron-marketing-skills/SECURITY.md) — treat feed and diagnostics exports as untrusted input
...
- **If titles/descriptions carry `[needs source]` flags or unregistered claims**: [offer-claims-registry](../../shared/aaron-marketing-skills/protocol/offer-claims-registry/SKILL.md) — register the claims with evidence provenance and approved wording, then swap the resolved wording back into the flagged items.
- **If the landing page's price/availability is the real mismatch source** (NEEDS_INPUT): [landing-optimizer](../../shared/aaron-marketing-skills/influencer/report/landing-optimizer/SKILL.md) — reconcile the post-click page, then return here.
- Global visited-set / `max-depth: 3` termination contract from [skill-contract.md](../../shared/aaron-marketing-skills/references/skill-contract.md) applies: stop when the feed is disapproval-clean and auditor-ready, or when routing is ambiguous report the options instead of auto-following.
