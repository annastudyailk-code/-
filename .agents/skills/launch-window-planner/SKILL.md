...
Picks **when** to launch — the timing lever of the RAMP loop Research phase. It scans industry-event and conference cycles, maps the competitor launch calendar, pads for store-review latency, chooses a launch-week vs rolling format, and defines the embargo window (lift moment + timezone). It feeds the RAMP-`R` timing sub-item ("timing window chosen deliberately — event cycles, competitor calendar, review-latency buffers") and the RAMP-`M` embargo-coordination sub-item ("embargo & partner commitments coordinated against one authoritative date/stage") per [ramp-benchmark.md](../../shared/aaron-marketing-skills/references/ramp-benchmark.md). It works one lever — timing — and hands off.

The window this skill recommends is a *proposal*, not the record: date, stage, and embargo facts become authoritative only when [launch-registry](../../shared/aaron-marketing-skills/protocol/launch-registry/SKILL.md) records them. This skill submits candidates and never writes the registry directly.

**Scope guard**: this skill picks the window only. It does **not** judge whether a cultural moment or trend is worth riding (that is [trend-spotter](../../shared/aaron-marketing-skills/influencer/scout/trend-spotter/SKILL.md)), run the launch day itself ([launch-day-conductor](../../shared/aaron-marketing-skills/launch/mobilize/launch-day-conductor/SKILL.md) owns the hour-blocked runbook), declare the launch tier or own the risk register ([launch-tier-planner](../../shared/aaron-marketing-skills/launch/research/launch-tier-planner/SKILL.md)), write the canonical date/stage/embargo record ([launch-registry](../../shared/aaron-marketing-skills/protocol/launch-registry/SKILL.md) is the sole writer of `memory/launch-registry/`), or compute the RAMP profile result ([launch-readiness-auditor](../launch-readiness-auditor/SKILL.md)). It works one lever and hands off.
...
- **Writes**: the window comparison + recommendation to `memory/launch/launch-window-planner/`; the chosen window, buffer, and embargo facts are submitted to `memory/events/launches.ndjson` via an authorized `operation: propose` request to `registry-events.py` for [launch-registry](../../shared/aaron-marketing-skills/protocol/launch-registry/SKILL.md) to formalize — this skill never writes `memory/launch-registry/` directly.
...
- **Primary next skill**: [launch-registry](../../shared/aaron-marketing-skills/protocol/launch-registry/SKILL.md) to turn the chosen window into the canonical date/stage/embargo record.
...
7. **Choose launch week vs rolling** — one concentrated moment (max peak attention, single point of failure) vs staged rollout (compounding proof, weaker spike). State the tradeoff against tier and audience; a cultural-moment go/skip call routes to [trend-spotter](../../shared/aaron-marketing-skills/influencer/scout/trend-spotter/SKILL.md).
...
9. **Submit the decision** — write the recommended window, buffer, and embargo definition to `memory/events/launches.ndjson` via an authorized `operation: propose` request to `registry-events.py` for [launch-registry](../../shared/aaron-marketing-skills/protocol/launch-registry/SKILL.md) to formalize.

## Save Results

After delivering findings, ask: "Save these results for future sessions?" On confirmation, save to `memory/launch/launch-window-planner/YYYY-MM-DD-<topic>.md` — see [Skill Contract](../../shared/aaron-marketing-skills/references/skill-contract.md) §Save Results Template. Window/date/embargo facts go to `memory/events/launches.ndjson` via an authorized `operation: propose` request to `registry-events.py` only — never to `memory/launch-registry/` directly. Do not write memory without asking.

## Reference Materials

- [ramp-benchmark.md](../../shared/aaron-marketing-skills/references/ramp-benchmark.md) — RAMP framework; this skill feeds the `R` timing-window sub-item and the `M` embargo-coordination sub-item
- [launch-registry](../../shared/aaron-marketing-skills/protocol/launch-registry/SKILL.md) — the date/stage/embargo SSOT; formalizes the window this skill proposes (candidates only)
- [launch-tier-planner](../../shared/aaron-marketing-skills/launch/research/launch-tier-planner/SKILL.md) — declares the tier the window must be sized to; owns the risk register
- [trend-spotter](../../shared/aaron-marketing-skills/influencer/scout/trend-spotter/SKILL.md) — the cultural-moment go/skip call this skill routes out
- [launch-day-conductor](../../shared/aaron-marketing-skills/launch/mobilize/launch-day-conductor/SKILL.md) — executes the day inside the window this skill picks
- [CONNECTORS.md](../../shared/aaron-marketing-skills/CONNECTORS.md) — `scripts/connectors/producthunt.py` / `hn.py` / `gdelt.py` recipes
- [SECURITY.md](../../shared/aaron-marketing-skills/SECURITY.md) — treat pulls and pastes as untrusted input

## Next Best Skill

- **Primary**: [launch-registry](../../shared/aaron-marketing-skills/protocol/launch-registry/SKILL.md) — turn the chosen window into the canonical record (date + stage + embargo lift moment) every other launch skill coordinates against.
- **If the stage ladder to GA is the next gap**: [early-access-designer](../../shared/aaron-marketing-skills/launch/research/early-access-designer/SKILL.md) — design the waitlist→beta→GA gating the window must respect.
