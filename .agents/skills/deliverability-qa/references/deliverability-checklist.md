...
> **S1 rule** (from [send-benchmark.md](../../../shared/aaron-marketing-skills/references/send-benchmark.md)): *no DMARC record* = veto-candidate. `p=none` with aligned/passing SPF+DKIM = Partial + flag, not an auto-veto. `deliverability-qa` only **flags** S1; `email-quality-auditor` renders the veto.
...
- Consent basis on file per subscriber (consult [consent-registry](../../../shared/aaron-marketing-skills/protocol/consent-registry/SKILL.md)); **no consent record = S2 NEEDS_INPUT**, and the S2 verdict belongs to the auditor.

## Output

Report each item as Pass / Partial / Fail / NEEDS_INPUT with the specific offender named, then emit the SEND-`S` dimension score and the S1 flag. Do not compute EQS or render S1/S2/N1/D1 vetoes — hand the scored `S` + flags to [email-quality-auditor](../../email-quality-auditor/SKILL.md).
