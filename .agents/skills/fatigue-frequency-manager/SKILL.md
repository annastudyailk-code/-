...
Ask "Save these results?" If yes, write to `memory/ad/fatigue-frequency-manager/` using `YYYY-MM-DD-<ad-set>-fatigue.md` — see [Skill Contract](../../shared/aaron-marketing-skills/references/skill-contract.md) §Save Results Template. This skill asks before writing memory and hands off veto-like measurement risks to `ad-account-auditor` rather than marking a veto itself.

## Reference Materials

- [ROAS Benchmark](../../shared/aaron-marketing-skills/references/roas-benchmark.md) — the paid-ads scoring framework; this skill works the **S** (CTR/CVR/frequency decay under spend-efficiency) and **R** (return-protection) levers; the Return vetoes R1/R2 govern whether a CVR-based read is trustworthy. Only `ad-account-auditor` computes the RQS or runs vetoes.
- [Measurement & Attribution Protocol](../../shared/aaron-marketing-skills/references/measurement-protocol.md) — baseline windows, conversion-lag handling, and frequency-ceiling guidance by objective.
- [scripts/connectors/README.md](../../shared/aaron-marketing-skills/scripts/connectors/README.md) — `ledger.py` record / trend reference for the decay slope.
...
Visited-set and `max-depth: 3` termination rules apply per [Skill Contract](../../shared/aaron-marketing-skills/references/skill-contract.md); if the recommended target was already run this chain, STOP and report chain-complete.
